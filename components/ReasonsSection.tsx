...
function ReasonCard({ reason, index, photo }: { reason: (typeof REASONS)[0]; index: number; photo: string }) {
  ...
  return (
    <motion.div
      ...
      className="flip-card h-36 sm:h-48"
      onClick={() => setFlipped((f) => !f)}
    >
      <div className={`flip-card-inner ${flipped ? 'flipped' : ''}`}>
        {/* Front */}
        <div className="flip-card-front glass flex flex-col items-center justify-center p-4 sm:p-6 cursor-pointer group overflow-hidden">
          {/* Number */}
          <div
            className="absolute top-2 sm:top-3 right-3 sm:right-4 font-playfair text-love-pink/30 font-bold z-10"
          ...
          <motion.div
            ...
            className="text-3xl sm:text-5xl mb-2 sm:mb-3 z-10"
          >
            {reason.emoji}
          </motion.div>

          <h3 className="font-playfair text-love-cream text-center font-semibold text-xs sm:text-base leading-tight z-10">
            {reason.title}
          </h3>

          <p className="text-love-rose/50 text-[10px] sm:text-xs mt-1 sm:mt-2 font-cormorant z-10">Nhấn để xem ❤️</p>
        </div>

        {/* Back — with photo background */}
        <div
          className="flip-card-back flex items-center justify-center p-3 sm:p-6 cursor-pointer overflow-hidden"
          style={{
        ...
        >
          ...
          <div className="text-center relative z-10">
            <div className="text-xl sm:text-2xl mb-2 sm:mb-3">{reason.emoji}</div>
            <p className="font-dancing text-love-cream/90 leading-relaxed text-xs sm:text-base" style={{ fontSize: 'clamp(0.7rem, 2.5vw, 1.05rem)' }}>
              &quot;{reason.desc}&quot;
            </p>
...
export default function ReasonsSection() {
  ...
  return (
    <section id="reasons" className="relative py-12 sm:py-24 px-3 sm:px-6 overflow-hidden"
    ...
    >
      ...
      <div ref={titleRef} className="text-center mb-8 sm:mb-16">
        ...
        <motion.p
          ...
          className="font-cormorant text-love-blush/60 mt-3 sm:mt-4 text-base sm:text-lg"
        >
      ...
      {/* Bottom counter */}
      <div className="flex justify-center mt-10 sm:mt-16 px-3 sm:px-4">
        <motion.div
          initial={{ opacity: 0, scale: 0.8 }}
          whileInView={{ opacity: 1, scale: 1 }}
          viewport={{ once: true }}
          transition={{ delay: 0.5 }}
          className="glass-dark rounded-2xl sm:rounded-full px-5 sm:px-10 py-3 sm:py-5 flex flex-col sm:flex-row items-center gap-2 sm:gap-4 text-center"
        >
          <span className="text-xl sm:text-3xl animate-heart-beat">❤️</span>
          <span className="font-dancing text-love-cream text-sm sm:text-xl">
